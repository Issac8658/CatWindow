...
namespace FFmpeg
{
	...
	public partial class FFmpegPlayer : Node // named so because otherwise Godot would not see it
	{
		...
		private ulong _bufferedFrames = 0; // in frames
		private ulong _trueDuration = 0; // calced duration after first loop, in frames
		...
		public double PlaybackPosition
		{
			get
			{
				...
				if (Metadata != null && Metadata.Format != null)
				{
					double Duration;
					if (_trueDuration == 0)
						Duration = double.Parse(Metadata.Format.Duration, CultureInfo.InvariantCulture);
					else
						Duration = FFmpeg.FrameToTime(_trueDuration);
					if (Duration != 0)
						PlayingTime -= Mathf.Floor(PlayingTime / Duration) * Duration;
				}
				return PlayingTime;
			}
		}
		...
		private void _Play(string input, double StartOffset)
		{
			...
			_bufferLength = _playback.GetFramesAvailable();
			_currentFile = input;
	...
	public static class FFmpeg
	{
		...
		*/
		...
		public static double FrameToTime(ulong FramesCount)
		{
			return (double)FramesCount / SAMPLE_RATE / CHANNELS;
		}
	}
